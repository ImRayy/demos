...
function App() {
  const [previewImages, setPreviewImages] = useState(images.slice(0, PAGE_SIZE));
  ...
  return (
    <div className="w-full min-h-screen bg-zinc-950">
      <div className="px-2 py-2 bg-secondary inline-flex items-center fixed z-10 right-4 top-4">{previewImages.length}/{images.length}</div>
      <div className="w-full mx-auto max-w-3xl pt-20 space-y-5">
        ...
        {hasMore && (
          <button onClick={loadMore} className="px-4 py-2 rounded bg-zinc-800 text-zinc-100">
            Load More
          </button>
        )}
      </div>
